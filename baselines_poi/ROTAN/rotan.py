...
class TransformerModel(nn.Module):
    def __init__(self, num_poi, embed_size, nhead, nhid, nlayers, target_time_dim, args, dropout=0.5):
        ...
        encoder_layers1 = TransformerEncoderLayer(args.user_embed_dim + args.poi_embed_dim, nhead, nhid, dropout,
                                                  batch_first=True)
        self.transformer_encoder1 = TransformerEncoder(encoder_layers1, nlayers)

        encoder_layers2 = TransformerEncoderLayer(args.poi_embed_dim + args.gps_embed_dim, nhead, nhid, dropout,
                                                  batch_first=True)
    ...
    def forward(self, src1, src2, src_mask, target_hour, target_day, poi_embeds, gps_embeds):
        ...
        src2_hour = rotate_batch(src2, target_hour[:, :, user_time_dim:], 2 * self.args.time_embed_dim,
                                 self.args.device)
...
class ROTAN(nn.Module):
    def __init__(self, num_users, num_pois):
        ...
        seq_model = TransformerModel(num_pois, args.seq_input_embed, args.transformer_nhead,args.transformer_nhid,args.transformer_nlayers,args.time_embed_dim,args,dropout=0.4)
