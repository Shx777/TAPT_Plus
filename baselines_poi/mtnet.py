...
class TreeLSTM(nn.Module):
    def __init__(self, num_users, num_POIs, **kwargs):
        ...
        self.nary = kwargs['nary']
        # embedding layer
        self.embedding_dim = kwargs['d_model'] + kwargs['d_model'] + kwargs['d_model'] + kwargs['d_model']
        self.user_embedding = nn.Embedding(num_embeddings=num_users, embedding_dim=kwargs['d_model'])
        self.POI_embedding = nn.Embedding(num_embeddings=num_POIs, embedding_dim=kwargs['d_model'])
        self.coo_embedding = nn.Embedding(num_embeddings=kwargs['K_cluster'], embedding_dim=kwargs['d_model'])
        # positional encoding layer
        self.time_pos_encoder = nn.Embedding(num_embeddings=96, embedding_dim=self.embedding_dim)  # 24*4
        # dropout layer
        self.embed_dropout = nn.Dropout(kwargs['embed_dropout'])
        self.model_dropout = nn.Dropout(kwargs['model_dropout'])
        # LSTM cell
        self.cell_IAC = IntraHierarchyCommunication(self.embedding_dim, kwargs['h_size'], kwargs['nary'])
        self.cell_IRC = InterHierarchyCommunication(kwargs['h_size'], kwargs['nary'])
        # decoder layer
        self.decoder_POI = nn.Linear(kwargs['h_size'], num_POIs)
