...
class TrainFactory:
    def __init__(self, model, exp_config, model_config, train_config, test_config):
        ...
        self.n_poi, self.n_user, self.train_loader, self.test_loader = self.prepare_data()
    ...
    def prepare_data(self):
        ...
        train_loader = DataLoader(dataset=train_data, batch_size=train_batch_size,
                                  ...
                                  num_workers=12,
                                  prefetch_factor=2,
                                  collate_fn=lambda e: gen_train_batch(e, train_data, max_len, n_poi, n_user))
        test_loader = DataLoader(dataset=test_data,
                                 batch_size=test_batch_size,
                                 num_workers=12,
                                 prefetch_factor=2,
                                 collate_fn=lambda e: gen_eval_batch(e, test_data, max_len, n_poi, n_user))
        return n_poi, n_user, train_loader, test_loader
    ...
    def train_function(self, dataloader, model, optimizer, epoch):
        ...
        for batch_idx, (src, trg, dsz, batch_data) in batch_iterator:
            optimizer.zero_grad()
            src = src.to(device)
            trg = trg.to(device)
            dsz = dsz.to(device)
            #batch_data = batch_data.to(device)
            logits = model(batch_data, dsz)
    ...
    def test_function(self, n_poi, dataloader, model, epoch):
        ...
        batch_iterator = tqdm(enumerate(dataloader), total=len(dataloader), leave=False, desc=tqdm_title)
        with torch.no_grad():
            for batch_idx, (src, trg, dsz, b_d) in batch_iterator:
                src = src.to(device)
                trg = trg.to(device)
                dsz = dsz.to(device)
                #b_d = b_d.to(device)
                scores = model(b_d, dsz)
